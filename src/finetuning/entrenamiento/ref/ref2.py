...
from peft import PeftModel, PeftConfig  # <-- añadido
...
if __name__ == "__main__":
    ...
    training_args = RewardConfig(
        output_dir="galicIA-v1-rm-iter1",  # <-- NUEVO: no pisa el original
    ...
    # Descubrir la base real del adapter y cargar tokenizer+backbone
    ADAPTER_PATH = model_args.model_name_or_path
    peft_cfg = PeftConfig.from_pretrained(ADAPTER_PATH)
    base_name = peft_cfg.base_model_name_or_path  # base del adapter LoRA

    tokenizer = AutoTokenizer.from_pretrained(
        base_name, trust_remote_code=model_args.trust_remote_code, use_fast=True
    )
    backbone = AutoModelForSequenceClassification.from_pretrained(
        base_name, num_labels=1, trust_remote_code=model_args.trust_remote_code, **model_kwargs
    )

    # Montar el LoRA existente y dejarlo entrenable
    model = PeftModel.from_pretrained(backbone, ADAPTER_PATH, is_trainable=True)
    ...
    # Asegurar que la cabeza de recompensa también se entrena
    base = model.get_base_model()
    reward_head = getattr(base, "score", None) or getattr(base, "classifier", None)
    ...
    # Cargar solo el 10% de forma aleatoria (nota: el factor actual es 0.00005)
    ...
    if needs_prefs:
        pref_ds = build_preference_dataset(
            raw_dataset,
            gen_model_name=base_name,  # usar la base para generar candidatos
    ...
    trainer = RewardTrainer(
        ...
        peft_config=None,  # ya es un PeftModel; entrenamos SOLO el LoRA existente (+ head)
    )
    trainer.train()
    ...
    # -----------------------------
    trainer.save_model(training_args.output_dir)  # guarda adapters LoRA (y config) en un dir NUEVO
